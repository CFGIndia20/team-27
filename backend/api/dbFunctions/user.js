...
module.exports = {
    ...
    findUserById: (_id) => {
        return User.findOne({ _id }).populate({path: 'student'}).populate({path: 'teacher'});
    },
    ...
    getAllUnverified: () => {
        return User.find({verified: false, access: {$ne: 'admin'}}).select('name email _id student access').populate({path: 'student'});
    }
}